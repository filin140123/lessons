...
class Item:
    ...
    def display_names(self, surface, name, cost, selected):
        ...
        cost_surface = self.font.render(f"cost: {int(cost)} EXP", False, color)
    ...
    def trigger(self, player):
        upg_attr = list(player.stats.keys())[self.index]

        if player.exp >= player.upgrade_cost[upg_attr] and player.stats[upg_attr] < player.max_stats[upg_attr]:
            player.exp -= player.upgrade_cost[upg_attr]
            player.stats[upg_attr] *= 1.2
            player.upgrade_cost[upg_attr] *= 1.4

        if player.stats[upg_attr] > player.max_stats[upg_attr]:
            player.stats[upg_attr] = player.max_stats[upg_attr]

    def display(self, surface, selection_number, name, value, max_value, cost):
        if selected := self.index == selection_number:
